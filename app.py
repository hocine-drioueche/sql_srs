...
if "exercises_sql_tables.duckdb" not in os.listdir("data"):
    exec(open("init_db.py").read())  # Exécute le fichier init_db.py pour créer la base de données
...
def check_users_solution(user_query: str) -> None:
    ...
    """
    result = con.execute(user_query).df()  # Exécute la requête de l'utilisateur et stocke le résultat
    st.dataframe(result)  # Affiche le résultat dans Streamlit
    try:
        # Compare les colonnes de la solution avec celles du résultat
        result = result[solution_df.columns]
        st.dataframe(result.compare(solution_df))  # Affiche les différences entre les deux dataframes
...
with st.sidebar:
    available_themes_df = con.execute("SELECT DISTINCT theme FROM memory_state").df()  # Récupère tous les thèmes disponibles
    theme = st.selectbox(
...
query = form.text_area(label="Votre code SQL ici", key="user_input")  # Champ de texte pour la requête
form.form_submit_button("Soumettre")  # Bouton de soumission

if query:
    check_users_solution(query)  # Appelle la fonction pour vérifier la solution de l'utilisateur
